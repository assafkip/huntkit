...
- "3 retries": Something is broken. Diagnose the root cause. Tell the user.
...
- "N edit attempts on [file]": Your edit approach is wrong. Read the file again, find the exact match, or ask the user.
...
- After 10 tool calls, pause and check: "Am I closer to the goal than 10 calls ago?" If not, stop and tell the user.
...
- If you've read 5+ files without writing anything, stop and tell the user what you're looking for and why.
- Never hold large API responses in context. Process and discard immediately.
- When blocked, do NOT brute-force. Try a different approach or ask the user.
