...
VOLUME_CEILING_ANALYZE = 200  # Volume ceiling when /analyze skill is active
VOLUME_WARNING = 35         # Tool calls since last user message = warn
VOLUME_WARNING_ANALYZE = 150  # Volume warning when /analyze skill is active
AGENT_CEILING = 3           # Agent spawns per session = block (raised to 25 during /analyze)
AGENT_CEILING_ANALYZE = 25  # Agent ceiling when /analyze skill is active
...
def parse_transcript(transcript_path, cache):
    ...
    analyze_active = cache.get("analyze_active", False)
    ...
    try:
        with open(transcript_path, "rb") as f:
            ...
            for line in f:
                ...
                # Detect /analyze skill activation
                if msg_type == "tool_use" or entry.get("type") == "tool_use":
                    entry_name = entry.get("name", "")
                    entry_input = entry.get("input", {})
                    if entry_name == "Skill" and entry_input.get("skill", "") == "analyze":
                        analyze_active = True
    ...
    cache["analyze_active"] = analyze_active
...
def check_volume(cache):
    """Block at ceiling, warn at warning threshold. Uses higher limits during /analyze."""
    analyze_active = cache.get("analyze_active", False)
    ceiling = VOLUME_CEILING_ANALYZE if analyze_active else VOLUME_CEILING
    warning = VOLUME_WARNING_ANALYZE if analyze_active else VOLUME_WARNING
    calls = cache.get("tool_calls_since_user", 0) + 1  # Include current

    if calls >= ceiling:
        return ("block", f"{ceiling} tool calls without user input. Stop. Summarize what you've accomplished and what's remaining.")

    if calls >= warning and cache.get("warnings_issued", 0) == 0:
        remaining = ceiling - calls
...
def check_agent_ceiling(tool_name, cache):
    """Block if too many agents spawned. Uses higher ceiling during /analyze."""
    if tool_name != "Agent":
        return None
    # /analyze skill spawns many agents by design (evidence collection,
    # technique dispatch, report synthesis). Use higher ceiling when active.
    analyze_active = cache.get("analyze_active", False)
    ceiling = AGENT_CEILING_ANALYZE if analyze_active else AGENT_CEILING
    # Include current call
    count = cache.get("agent_calls", 0) + 1
    if count > ceiling:
        return f"{ceiling} subagents already spawned this session. Use direct tool calls (Grep, Glob, Read) instead."
    return None
...
def main():
    ...
    # Detect /analyze skill activation from current tool call
    # (transcript parsing catches past calls; this catches the current one)
    if tool_name == "Skill" and tool_input.get("skill", "") == "analyze":
        cache["analyze_active"] = True
        # Signal to tool-counter.sh (separate hook, can't share cache)
        try:
            flag = f"/tmp/claude-analyze-active-{time.strftime('%Y%m%d')}.flag"
